...
py::tuple from_points(py::array_t<float, py::array::c_style | py::array::forcecast> points_np) {
    ...
    // Create degenerate triangles for each point
    std::vector<tinybvh::bvhvec4> vertices;
...
PYBIND11_MODULE(pytinybvh, m) {
    ...
    m.def("from_points", &from_points, "Builds a BVH for points from a (N, 3) float array.");
}